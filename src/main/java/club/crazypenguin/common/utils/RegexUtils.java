...
public class RegexUtils {
    ...
    public static void main(String[] args) {
        ...
        //TODO 使用jakarta-regexp
//        RE re = new RE(); // 新建正则表达式对象;
//        RECompiler compiler = new RECompiler(); // 新建编译对象;
//        re.setProgram(compiler.compile(LETTER_REGEXP)); // 编译
...
//        }
        ...
        String somePatternString="\\b\\d+\\b";
...
//            }
        ...
        RE re = new RE(somePatternString);

        boolean isTheRegMatch = re.match(someStringInput);
        if (isTheRegMatch) {
        ...
        Matcher testPreMatcher = Pattern.compile(somePatternString)
                .matcher(someStringInput);
        int groutCount = testPreMatcher.groupCount();
        System.out.println("匹配数 :" +groutCount);

        if (testPreMatcher.find()) {
            for (int j = 0; j < (groutCount + 1); j++) {
                System.out.println("Item " +j + ":" + testPreMatcher.group(j));
            }
        }
    }
